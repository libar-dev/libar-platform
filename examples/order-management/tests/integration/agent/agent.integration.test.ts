 ...
 *
 * Most tests require OPENROUTER_INTEGRATION_TEST_API_KEY because the agent
 * requires LLM analysis (no mock fallback — 22d design decision).
 * Dead letter infrastructure tests run without an API key.
 */
    ...
    describe("Checkpoint Persistence", () => {
      it("should create checkpoint on first event processing", async () => {
        ...
        await waitUntil(
          ...
          { message: "Order cancelled by saga compensation", timeoutMs: AGENT_TEST_TIMEOUT }
        );
        ...
        await waitUntil(
          ...
          { message: "Customer cancellations projection updated", timeoutMs: AGENT_TEST_TIMEOUT }
        );
        ...
        await waitUntil(
          ...
          { message: "Agent checkpoint created", timeoutMs: AGENT_TEST_TIMEOUT }
        );
      ...
      it("should detect churn risk after 3+ cancellations", async () => {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          await waitUntil(
            ...
            { message: `Order ${i + 1} cancelled by saga`, timeoutMs: AGENT_TEST_TIMEOUT }
          );
          ...
          await waitUntil(
            ...
            { message: `Customer cancellation ${i + 1} recorded`, timeoutMs: AGENT_TEST_TIMEOUT }
          );
        }
    ...
    describe("Audit Trail", () => {
      ...
      it("should create audit events when churn pattern is detected", async () => {
        ...
        await waitUntil(
          ...
          { message: "Agent checkpoint updated", timeoutMs: AGENT_TEST_TIMEOUT }
        );
        ...
        await waitUntil(
          ...
          { message: "Audit events created", timeoutMs: AGENT_TEST_TIMEOUT }
        );
    ...
    describe("Active Agents Query", () => {
      it("should list active agents", async () => {
        ...
        await waitUntil(
          ...
          { message: "Order confirmed", timeoutMs: AGENT_TEST_TIMEOUT }
        );
        ...
        await waitUntil(
          ...
          { message: "Agent checkpoint exists", timeoutMs: AGENT_TEST_TIMEOUT }
        );
    ...
    describe("Workpool Partition Ordering", () => {
      ...
      it.skip("should process events for same customer in order", async () => {
        ...
        await waitUntil(
          ...
          { message: "All 5 customer cancellations recorded", timeoutMs: AGENT_TEST_TIMEOUT }
        );
        ...
        await waitUntil(
          ...
          { message: "Agent processed 5+ events", timeoutMs: AGENT_TEST_TIMEOUT }
        );
    ...
    describe("Projection-Based Pattern Detection", () => {
      ...
      it("should detect churn pattern using customerCancellations projection", async () => {
        ...
        await waitUntil(
          ...
          { message: "Agent audit events created", timeoutMs: AGENT_TEST_TIMEOUT }
        );
...
// ============================================================================
// Dead Letter Infrastructure Tests (no LLM required — uses test helpers)
// ============================================================================

describe("Agent Dead Letter Handling", () => {
  let t: ConvexTestingHelper;
