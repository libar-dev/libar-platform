...
const shouldSkipAgentTests = !process.env.OPENROUTER_INTEGRATION_TEST_API_KEY;

(shouldSkipAgentTests ? describe.skip : describe)("Churn Risk Agent Pipeline", () => {
  describeFeature(churnRiskFeature, ({ Scenario, Background, AfterEachScenario }) => {
    ...
    Scenario("Two cancellations do not trigger churn risk", ({ Given, And, When, Then }) => {
      ...
      When(
        "the churn-risk agent checkpoint has advanced past the cancellation events",
        async () => {
          // Projection is already up-to-date from sequential creation step.
          // Just wait for the agent checkpoint to advance.
          await waitUntil(
            async () => {
              const checkpoint = await testQuery(
                scenarioState!.t,
                api.queries.agent.getCheckpoint,
                {
                  agentId: CHURN_RISK_AGENT_ID,
                }
              );
              return checkpoint?.eventsProcessed && checkpoint.eventsProcessed >= 2
                ? checkpoint
                : null;
            },
            {
