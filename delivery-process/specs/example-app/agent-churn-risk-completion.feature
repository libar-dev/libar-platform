  ...
  **Infrastructure Completed by Phases 22a-22c:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | Churn-risk action handler | complete | order-management/convex/contexts/agent/handlers/analyzeEvent.ts | Yes | integration |
      | Churn-risk onComplete handler | complete | order-management/convex/contexts/agent/handlers/onComplete.ts | Yes | integration |
  ...
  Rule: LLM analysis is essential, not optional
    ...
    **Verified by:** LLM called on trigger, failure creates dead letter, no silent fallback
    ...
    Scenario: Three cancellations trigger LLM analysis via OpenRouter
    ...
    @acceptance-criteria @happy-path
    Scenario: High confidence triggers auto-execution of SuggestCustomerOutreach
    ...
    @acceptance-criteria @happy-path
    Scenario: Low confidence queues for human approval
    ...
    Scenario: LLM failure exhausts retries and creates dead letter
  ...
  # ============================================================================
  # RULE 3: Emitted Commands Create Real Domain Records
  # ============================================================================
  ...
  Rule: Emitted commands create real domain records
    ...
    **Verified by:** Command routes to handler, handler creates record, event emitted
    ...
    @acceptance-criteria @happy-path
    Scenario: Full end-to-end flow from cancellation to outreach record
    ...
    @acceptance-criteria @validation
    Scenario: Command with missing customerId fails validation
    ...
    @acceptance-criteria @edge-case
    Scenario: Command handler failure creates dead letter for operator triage
      Given the agent has emitted a SuggestCustomerOutreach command
