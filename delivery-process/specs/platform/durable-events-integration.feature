...
@libar-docs-business-value:production-reliability-and-failure-recovery
@libar-docs-depends-on:ProductionHardening,DurableFunctionAdapters,EventReplayInfrastructure
@libar-docs-executable-specs:order-management/tests/integration-features/durability/
Feature: Durable Events Integration - Production-Ready Event Persistence
  ...
  **PDR-008 Alignment:** This demonstrates platform capabilities in realistic scenarios,
  ...
  Rule: Events are appended idempotently using command-derived keys
    ...
    **Integration Location:**
    ...
    Scenario: First command creates event normally
    ...
    Scenario: Retry with same commandId returns existing event
    ...
    Scenario: Different commandId is rejected for already-submitted order
      Given an OrderSubmitted event exists with idempotencyKey "cmd:SubmitOrder:ord-123:cmd-456"
      ...
      # Note: Different commandId means different command instance, but business rule prevents
      # submitting an already-submitted order. Idempotency handles retries, not new commands.

  # ===========================================================================
  # RULE 2: Intent/Completion Bracketing
  # ===========================================================================

  Rule: Commands record intent before execution and completion after success
    ...
    **Schema Addition (commandIntents table):**
    ...
    @acceptance-criteria @failure-handling
    Scenario: Failed command records intent and failure
    ...
    @acceptance-criteria @orphan-detection
    Scenario: Orphaned intent detected by scheduled job
    ...
    Scenario: Intent already exists for commandId
  ...
  # ===========================================================================
  # RULE 3: Durable Event Publication
  # ===========================================================================

  Rule: Critical events use Workpool-backed durable append
    ...
    **Dead Letter Handling:**
    ...
    @acceptance-criteria @happy-path
    Scenario: Durable append succeeds on first try
    ...
    @acceptance-criteria @retry-semantics
    Scenario: Durable append retries on transient failure
    ...
    @acceptance-criteria @dead-letter
    ...
    @acceptance-criteria @partition-ordering
    Scenario: Multiple events for same entity maintain order
  ...
  # ===========================================================================
  # RULE 4: Outbox Handler for Action Results
  # ===========================================================================

  Rule: External action results are captured as events using outbox pattern
    ...
    **Implementation:**
    ...
    @acceptance-criteria @happy-path
    Scenario: Successful payment creates PaymentCompleted event
    ...
    @acceptance-criteria @failure-handling
    Scenario: Failed payment creates PaymentFailed event
      Given an order "ord-123" awaiting payment
    ...
    Scenario: Duplicate completion is deduplicated
  ...
  # ===========================================================================
  # RULE 5: Poison Event Handling in Production
  # ===========================================================================

  Rule: Projection handlers quarantine malformed events
    ...
    **Admin Mutations:**
    ...
    @acceptance-criteria @happy-path
    Scenario: Valid event processed normally
    ...
    @acceptance-criteria @quarantine
    Scenario: Malformed event quarantined after max attempts
    ...
    Scenario: Quarantined event can be replayed after fix
    ...
    Scenario: Quarantined event can be ignored
  ...
  # ===========================================================================
  # RULE 6: Projection Rebuild Demonstration
  # ===========================================================================

  Rule: Projections can be rebuilt from the event stream
    ...
    **Progress Tracking Structure:**
    ...
    @acceptance-criteria @happy-path
    Scenario: Rebuild from position 0 re-processes all events
    ...
    Scenario: Rebuild progress is trackable
    ...
    Scenario: Running rebuild can be cancelled
      Given a rebuild in progress for orderSummary
    ...
    @acceptance-criteria @concurrent-prevention
    Scenario: Concurrent rebuilds are prevented
    ...
    @acceptance-criteria @partial-rebuild
    Scenario: Rebuild from specific position
  ...
  # ===========================================================================
  # RULE 7: Orphan Intent Detection
  # ===========================================================================

  Rule: Scheduled job detects and alerts on orphaned command intents
    ...
    **Detection Logic:**
    ...
    Scenario: Orphan detected after threshold exceeded
    ...
    @acceptance-criteria @no-false-positives
    Scenario: Recent pending intent not flagged
      Given a commandIntent with status "pending"
      And timeoutMs is 300000 (5 minutes)
    ...
    @acceptance-criteria @completed-ignored
    Scenario: Completed intents are ignored
    ...
    Scenario: Orphan detection reports metrics
  ...
  Rule: End-to-end durability is verified via integration tests
    ...
    **Test Infrastructure:**
    ...
    @integration @smoke
    Scenario: Full durable command flow
    ...
    Scenario: Command retry produces same result
    ...
    @integration @rebuild
    Scenario: Projection rebuild restores correct state
