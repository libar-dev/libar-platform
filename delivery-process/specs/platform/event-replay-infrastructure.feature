...
@libar-docs-phase:18b-1
@libar-docs-effort:1w
@libar-docs-product-area:Platform
@libar-docs-business-value:projection-recovery-and-schema-migration
@libar-docs-depends-on:EventStoreFoundation,DurableFunctionAdapters,EventStoreDurability
@libar-docs-enables:SchemaMigration,ProjectionRecovery
@libar-docs-executable-specs:platform-core/tests/features/behavior/event-replay
@libar-docs-see-also:EventStoreDurability,ProjectionCategories,WorkpoolPartitioningStrategy
  ...
  **Relationship to Existing Patterns:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | replayCheckpoints table schema | complete | examples/order-management/convex/schema.ts | No | - |
      | Replay types and interfaces | complete | @libar-dev/platform-core/src/projections/replay/types.ts | Yes | unit |
      | Replay progress calculator | complete | @libar-dev/platform-core/src/projections/replay/progress.ts | Yes | unit |
      | eventReplayPool Workpool config | complete | examples/order-management/convex/infrastructure.ts | No | - |
      | triggerRebuild mutation | complete | examples/order-management/convex/admin/projections.ts | Yes | integration |
      | cancelRebuild mutation | complete | examples/order-management/convex/admin/projections.ts | Yes | integration |
      | getRebuildStatus query | complete | examples/order-management/convex/admin/projections.ts | Yes | integration |
      | listActiveRebuilds query | complete | examples/order-management/convex/admin/projections.ts | Yes | integration |
      | processReplayChunk internal mutation | complete | examples/order-management/convex/admin/projections.ts | Yes | integration |
      | Replay chunk size configuration | complete | examples/order-management/convex/admin/projections.ts | No | - |

  # ===========================================================================
  # RULE 1: Checkpoint-Based Resumption
  # ===========================================================================

  Rule: Replay must resume from last successful checkpoint
    ...
    **Target State (checkpoint-based replay):**
    ...
    Scenario: Replay resumes after failure
    ...
    Scenario: Checkpoint updates atomically with chunk completion
    ...
    Scenario: Replay handles empty event range
  ...
  # ===========================================================================
  # RULE 2: Dedicated Low-Priority Workpool
  # ===========================================================================

  Rule: Replay uses dedicated low-priority Workpool
    ...
    **Target Implementation:**
    ...
    @acceptance-criteria @happy-path
    Scenario: Replay does not starve live projections
    ...
    @acceptance-criteria @validation
    Scenario: Only one replay per projection
    ...
    @acceptance-criteria @validation
    Scenario: Different projections can rebuild concurrently
  ...
  # ===========================================================================
  # RULE 3: Chunked Processing
  # ===========================================================================

  Rule: Events are processed in configurable chunks
    ...
    **Target Implementation:**
    ...
    @acceptance-criteria @happy-path
    Scenario: Chunk processes correct number of events
    ...
    @acceptance-criteria @validation
    Scenario: Chunk size respects projection complexity
  ...
  # ===========================================================================
  # RULE 4: Progress Visibility
  # ===========================================================================

  Rule: Replay progress is queryable in real-time
    ...
    **Progress Calculation:**
    ...
    @acceptance-criteria @happy-path
    Scenario: Query replay progress
      Given a replay is running for "orderSummary"
    ...
    @acceptance-criteria @happy-path
    Scenario: List all active rebuilds
    ...
    @acceptance-criteria @validation
    Scenario: Progress handles completed replay
  ...
  # ===========================================================================
  # RULE 5: Admin Operations
  # ===========================================================================

  Rule: Admin mutations enable operational control
    ...
    **Target Implementation:**
    ...
    @acceptance-criteria @happy-path
    Scenario: Trigger rebuild creates checkpoint and schedules first chunk
    ...
    @acceptance-criteria @happy-path
    Scenario: Cancel rebuild stops processing
      Given a replay is running for "orderSummary" with replayId "replay-001"
    ...
    @acceptance-criteria @validation
    Scenario: Cannot trigger duplicate rebuild
      Given a replay is running for "orderSummary"
