...
@libar-docs-pattern:PackageArchitecture
@libar-docs-status:completed
@libar-docs-unlock-reason:Metadata-normalization-for-v0.2.0-release-alignment
@libar-docs-phase:13
@libar-docs-quarter:Q1-2026
@libar-docs-effort:3w
@libar-docs-completed:2026-01-08
@libar-docs-product-area:Platform
@libar-docs-business-value:focused-packages-for-consumers-with-clear-boundaries
@libar-docs-priority:high
Feature: Package Architecture - Extraction and Naming Strategy
  ...
  **Known Gaps (deferred):**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | @libar-dev/platform-fsm package | complete | Yes | deps/libar-dev-packages/packages/platform/fsm/ |
      | @libar-dev/platform-decider package | complete | Yes | deps/libar-dev-packages/packages/platform/decider/ |
      | @libar-dev/platform-store package | complete | Yes | deps/libar-dev-packages/packages/platform/store/ |
      | @libar-dev/platform-bus package | complete | Yes | deps/libar-dev-packages/packages/platform/bus/ |
      | @libar-dev/platform-core package | complete | Yes | deps/libar-dev-packages/packages/platform/core/ |
      | @libar-dev/platform-bc package | complete | Yes | deps/libar-dev-packages/packages/platform/bc/ |
      | Layer 0 zero-dependency enforcement | complete | No | package.json validation |
      | Decider test migration to package | complete | Yes | platform-decider/tests/features/behavior/ |
      | FSM test migration to package | complete | Yes | platform-fsm/tests/features/behavior/ |
      | Backward compatibility re-exports | complete | No | platform-core/src/decider/, platform-core/src/fsm/ |
      | @libar-dev/platform-dcb extraction | deferred | No | Future work |
      | @libar-dev/platform-orchestrator extraction | deferred | No | Future work |

  Rule: Layer 0 packages have no framework dependencies

    @acceptance-criteria
    Scenario: platform-fsm has no dependencies
    ...
    @acceptance-criteria
    Scenario: platform-decider depends only on platform-fsm
      Given the @libar-dev/platform-decider package
      When checking its dependencies
      Then it should only depend on @libar-dev/platform-fsm
      And it should have zero convex dependencies

  Rule: Consumers can install individual packages

    @acceptance-criteria
    Scenario: Decider package is independently usable
    ...
    @acceptance-criteria
    Scenario: Packages are independently publishable
  ...
  Rule: Tests ship with framework packages

    @acceptance-criteria
    Scenario: Framework tests live in framework packages
      Given the @libar-dev/platform-decider package
  ...
  Rule: Backward compatibility is maintained
  ...
  Rule: No naming conflicts with libar-ai project

    @technical-constraint
    Scenario: Platform namespace avoids conflicts
      ...
      # The libar-ai project uses: core, convex-commands, convex-workflows, zod-convex-*
