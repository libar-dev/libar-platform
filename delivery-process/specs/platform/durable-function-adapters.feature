...
@libar-docs-unlock-reason:deliverable-status-update-not-scope-creep
@libar-docs-phase:18a
@libar-docs-effort:3d
@libar-docs-product-area:Platform
@libar-docs-business-value:production-grade-reliability-with-minimal-code
@libar-docs-depends-on:DCB
@libar-docs-executable-specs:platform-core/tests/features/behavior/durable-function-adapters
  ...
  **Architectural Context:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | RateLimitChecker types | complete | @libar-dev/platform-core/src/middleware/types.ts | Yes | unit |
      | Remove in-memory rate limiters | complete | @libar-dev/platform-core/src/middleware/ | No | - |
      | ConvexRateLimitAdapter | complete | @libar-dev/platform-core/src/middleware/rateLimitAdapter.ts | Yes | integration |
      | Rate limiter component mount | complete | examples/order-management/convex/convex.config.ts | Yes | integration |
      | Rate limit definitions | complete | examples/order-management/convex/rateLimits.ts | Yes | integration |
      | DCB retry types (reuses Workpool RetryBehavior) | complete | @libar-dev/platform-core/src/dcb/withRetry.ts | Yes | unit |
      | withDCBRetry helper | complete | @libar-dev/platform-core/src/dcb/withRetry.ts | Yes | integration |
      | DCB retry Workpool mount | complete | examples/order-management/convex/convex.config.ts | Yes | integration |
      | DCB retry internal mutation | complete | examples/order-management/convex/dcb/retryExecution.ts | Yes | integration |
      | Backoff calculation (standard formula) | complete | @libar-dev/platform-core/src/dcb/backoff.ts | Yes | unit |
      | Backoff jitter function (injectable) | complete | @libar-dev/platform-core/src/dcb/backoff.ts | Yes | unit |
      | DCB retry onComplete support | complete | @libar-dev/platform-core/src/dcb/withRetry.ts | Yes | integration |
      | Adapter usage documentation | complete | docs/architecture/DURABLE-FUNCTION-ADAPTERS.md | No | - |
  ...
  Rule: Rate limit adapter bridges middleware to component
    ...
    **Formula:** `shards ≈ QPS / 2`
    ...
    Scenario: Adapter isolates limits by key
    ...
    Scenario: Adapter integrates with existing middleware pipeline
  ...
  Rule: DCB retry helper automatically handles OCC conflicts
    ...
    **Backoff Calculation (standard formula from CONVEX-DURABILITY-REFERENCE.md):**
    ...
    @acceptance-criteria @happy-path
    Scenario: DCB conflict triggers automatic retry
    ...
    @acceptance-criteria @edge-case
    Scenario: Backoff increases exponentially with jitter
    ...
    @acceptance-criteria @edge-case
    Scenario: Version advances between retry scheduling and execution
  ...
  Rule: Adapters integrate with existing platform infrastructure
    ...
    **DCB Retry Integration in Example App:**
    ...
    @acceptance-criteria @integration
    Scenario: Rate limiter mounts as Convex component
    ...
    @acceptance-criteria @integration
    Scenario: DCB retry pool mounts as separate Workpool
    ...
    @acceptance-criteria @validation
    Scenario: Middleware pipeline order preserved
