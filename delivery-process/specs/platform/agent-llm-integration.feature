...
@libar-docs-pattern:AgentLLMIntegration
@libar-docs-status:active
@libar-docs-phase:22b
@libar-docs-effort:1w
@libar-docs-product-area:Platform
@libar-docs-depends-on:AgentBCComponentIsolation
@libar-docs-executable-specs:platform-core/tests/features/behavior/agent/llm-integration
  ...
  **Circuit Breaker Integration — Phase 18 Relationship:**
  ...
  Background: Deliverables
    Given the following deliverables:
      ...
      | Agent workpool configuration | pending | examples/order-management/convex/convex.config.ts | Yes | integration |
  ...
  Rule: Agent event handlers are actions for LLM integration
    ...
    **Verified by:** Action calls LLM, onComplete persists, fallback works, timeout handled
    ...
    Scenario: Agent action handler calls LLM and returns decision
    ...
    Scenario: Action handler rejects invalid agent configuration
  ...
  # ============================================================================
  # RULE 2: Rate Limiting Is Enforced Before LLM Calls
  # ============================================================================

  Rule: Rate limiting is enforced before LLM calls
    ...
    **Verified by:** Rate limit blocks excess calls, cost budget pauses agent,
    ...
    @acceptance-criteria @happy-path
    Scenario: Rate limiter allows LLM call within limits
    ...
    @acceptance-criteria @validation
    Scenario: Rate limiter blocks LLM call when exceeded
      Given an agent with rate limit of 60 requests per minute
  ...
  # ============================================================================
  # RULE 3: Agent Subscriptions Support onComplete Callbacks
  # ============================================================================

  Rule: Agent subscriptions support onComplete callbacks
    ...
    **Verified by:** onComplete receives callbacks, dead letters created on failure,
    ...
    @acceptance-criteria @happy-path
    Scenario: Agent subscription with onComplete receives completion callbacks
    ...
    @acceptance-criteria @happy-path
    Scenario: Failed agent jobs create dead letters via onComplete
      Given an agent subscription configured with custom onComplete handler
    ...
    @acceptance-criteria @validation
    Scenario: Agent subscription without onComplete uses global default
