...
// ============================================================================
// Evolved AgentActionResult (AD-6)
// ============================================================================

/**
 * Result returned by the agent action handler.
 *
 * EVOLUTION from DS-2: Added `patternId` field.
 *
 * When the action handler uses PatternExecutor (patterns mode), patternId
 * is set to the name of the pattern that produced the decision. This
 * flows through onComplete to commands.record for audit and correlation.
 *
 * In onEvent mode (legacy), patternId is undefined.
 *
 * IMPLEMENTATION NOTE: Extend the existing AgentActionResult in
 * delivery-process/stubs/agent-action-handler/action-handler.ts
 */
export interface AgentActionResult {
   ...
   * - "rule-based-fallback": LLM was configured but unavailable
   */
  readonly analysisMethod: "llm" | "rule-based" | "rule-based-fallback";

  /**
   * DS-4 ADDITION: Pattern that produced this decision.
   *
   * Set by PatternExecutor when using patterns mode.
   * Undefined in onEvent mode (legacy).
   *
   * Flows through: PatternExecutor → AgentActionResult → onComplete → commands.record
   */
