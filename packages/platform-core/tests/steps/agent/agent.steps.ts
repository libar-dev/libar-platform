 ...
 */
function getDataTableRows<T extends Record<string, string>>(
  dataTable: unknown
): T[] {
  if (!dataTable) return [];
...
function generateEventsForDays(days: number, eventType: string = "OrderCancelled"): PublishedEvent[] {
...
describeFeature(eventSubscriptionFeature, ({ Background, Rule, BeforeEachScenario, AfterEachScenario }) => {
  ...
  Rule("Agents subscribe to specific event types", ({ RuleScenario }) => {
    RuleScenario("Subscribe to single event type", ({ Given, When, Then, And }) => {
      ...
      When("an OrderSubmitted event is published", () => {
        ...
        if (shouldProcessAgentEvent(event.globalPosition, state.checkpoint!.lastProcessedPosition)) {
  ...
  Rule("Subscriptions support filters", ({ RuleScenario }) => {
    ...
    RuleScenario("Filter by customer segment", ({ Given, When, And, Then }) => {
      ...
      Then("the agent receives only the premium customer event", () => {
        expect(state.receivedEvents).toHaveLength(1);
        expect((state.receivedEvents[0].payload as { customer: { segment: string } }).customer.segment).toBe("premium");
      });
    });
  });
  ...
  Rule("Events are delivered in order", ({ RuleScenario }) => {
    ...
    RuleScenario("Events from same stream maintain order", ({ Given, When, Then }) => {
      ...
      Then("events are processed in sequence order", () => {
        const sequences = state.receivedEvents.map((e) => (e.payload as { sequence: number }).sequence);
        expect(sequences).toEqual([1, 2, 3]);
      });
    });

    RuleScenario("Agent resumes from last processed position after restart", ({ Given, And, When, Then }) => {
      Given("an agent subscribed to OrderSubmitted", () => {
        state.subscriptions = ["OrderSubmitted"];
      });
  ...
  Rule("Agent checkpoint tracks processing progress", ({ RuleScenario }) => {
    ...
    RuleScenario("Failed analysis records to dead letter without advancing checkpoint", ({ Given, And, When, Then }) => {
      Given("an agent subscribed to OrderSubmitted", () => {
        state.subscriptions = ["OrderSubmitted"];
      });
...
describeFeature(patternDetectionFeature, ({ Background, Rule, BeforeEachScenario, AfterEachScenario }) => {
  BeforeEachScenario(() => resetState());
  AfterEachScenario(() => resetState());
  ...
  Rule("Pattern window constrains event scope", ({ RuleScenario }) => {
    ...
    RuleScenario("Pattern window loads events lazily for memory efficiency", ({ Given, And, When, Then }) => {
  ...
  Rule("Patterns are detected from event sequences", ({ RuleScenario }) => {
    RuleScenario("Detect ChurnRisk from cancellations", ({ Given, When, Then, And }) => {
      ...
      And("confidence is calculated", () => {
        ...
        const cancellations = state.eventsInWindow.filter((e) => e.eventType === "OrderCancelled");
...
describeFeature(commandEmissionFeature, ({ Background, Rule, BeforeEachScenario, AfterEachScenario }) => {
  BeforeEachScenario(() => resetState());
  AfterEachScenario(() => resetState());
  ...
  Rule("Commands include explainability metadata", ({ RuleScenario }) => {
    ...
    RuleScenario("Command includes LLM context", ({ Given, When, Then }) => {
      ...
      Then("command.metadata.llmContext includes model, tokens, duration", () => {
        const analysis = state.emittedCommand!.metadata.analysis as { model: string; tokens: number; duration: number };
  ...
  Rule("Command emission handles LLM failures gracefully", ({ RuleScenario }) => {
    RuleScenario("LLM rate limit is handled with exponential backoff", ({ Given, And, When, Then }) => {
    ...
    RuleScenario("LLM timeout falls back to rule-based emission", ({ Given, And, When, Then }) => {
  ...
  Rule("Different command types for different actions", ({ RuleScenario }) => {
    ...
    RuleScenario("Emit action command", ({ Given, When, Then }) => {
      ...
      Then("command triggers actual business action", () => {
        ...
        expect((state.emittedCommand!.payload as { action: string }).action).toBe("apply-discount");
      });
    });
...
const humanInLoopFeature = await loadFeature(
  "tests/features/behavior/agent/human-in-loop.feature"
);

describeFeature(humanInLoopFeature, ({ Background, Rule, BeforeEachScenario, AfterEachScenario }) => {
  BeforeEachScenario(() => resetState());
  AfterEachScenario(() => resetState());
  ...
  Rule("Confidence threshold determines execution mode", ({ RuleScenario, RuleScenarioOutline }) => {
    RuleScenarioOutline(
...
const auditTrailFeature = await loadFeature(
  "tests/features/behavior/agent/audit-trail.feature"
);

describeFeature(auditTrailFeature, ({ Background, Rule, BeforeEachScenario, AfterEachScenario }) => {
  BeforeEachScenario(() => resetState());
  AfterEachScenario(() => resetState());
