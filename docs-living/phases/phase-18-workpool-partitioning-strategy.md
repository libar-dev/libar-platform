...
**Purpose:** Detailed patterns for WorkpoolPartitioningStrategy
...
- Depends on: ProductionHardening
- Depends on: DurableFunctionAdapters
- Depends on: EventReplayInfrastructure
...
_Verified by: DCB succeeds on first attempt, DCB conflict triggers automatic retry, Max retries exceeded returns rejected, Backoff increases exponentially with jitter, Scope-aware scheduling metadata is preserved, DCB retry with onComplete callback, Version advances between retry scheduling and execution_
...
_Verified by: Entity projection processes events in order, Different entities process in parallel, Entity partition key helper generates correct format_
...
_Verified by: Customer projection aggregates across orders, Customer partition key helper validates required field_
...
_Verified by: Global rollup processes sequentially, Global rollup avoids OCC conflicts, Dedicated Workpool alternative_
...
_Verified by: Cross-context projection maintains saga ordering, Different sagas process in parallel_
