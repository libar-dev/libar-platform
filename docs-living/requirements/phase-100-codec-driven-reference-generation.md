...
- ReferenceDocConfig type definition (pending)
- ConventionExtractor utility (pending)
- ReferenceDocumentCodec factory (pending)
- Reference generator registrations (pending)
- @libar-docs-convention tag in registry (pending)
- Convention values in taxonomy (pending)
- Decision record migration (existing ADRs) (pending)
- Decision record creation (new from recipe Rules) (pending)
- Recipe directory removal (pending)
- CLI integration (generate-docs --generators reference-\*) (pending)
- Dual output path support in generator (pending)
