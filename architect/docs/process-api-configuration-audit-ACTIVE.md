...
| --------------------------------- | ----- | ------ | ------------------------------------------------------------------- |
| Fixed (prior code changes)        | 1     | D-1    | 23 agent stubs: `* Target:` -> `@architect-target`                  |
...
**158 patterns** in the current dataset (up from 142 after earlier `@architect` opt-in fixes).
...
| **Tag prefix**  | `@architect-`                        | `@architect-`                                           |
...
| ------- | ---------------------------------------------- | ----------- | ---------- | ---------------------------------------------------------------------------- |
| **D-1** | 23 agent stubs with broken `* Target:` format  | **HIGH**    | **FIXED**  | `@architect-target` tags added to all 23 files                               |
...
| **D-5** | PDRs 011-013 missing `@architect-status`       | LOW         | DOCUMENTED | Pure decisions with `@architect-adr-status:accepted` only                    |
...
### D-1: Agent Stubs — `* Target:` vs `@architect-target` [FIXED]
...
matches the `@architect-target` JSDoc tag format, so `targetPath` was `undefined` for all
23 stubs.
...
- Moved target path from description body to `@architect-target` JSDoc tag
- Stripped parenthetical annotations from tag values (5 files had them)
- For `eventbus-publish-update.ts` (dual targets): primary target as `@architect-target`,
  secondary target preserved as `* Also modifies:` description line

**Edge cases handled:**

| File                          | Original Annotation                          | Clean `@architect-target`                               |
| ----------------------------- | -------------------------------------------- | ------------------------------------------------------- |
...
Both share the same `@architect-` tag prefix.

### D-5: PDRs 011-013 Missing `@architect-status` [DOCUMENTED]

**What:** Three newer PDRs (pdr-011-agent-action-handler-architecture, pdr-012-agent-command-routing,
pdr-013-agent-lifecycle-fsm) have `@architect` opt-in and `@architect-adr-status:accepted`
but do NOT have a `@architect-status` FSM tag.

**Where:** `libar-platform/delivery-process/decisions/pdr-011*.feature`, `pdr-012*.feature`, `pdr-013*.feature`

**Impact:** The Gherkin extractor requires `@architect-status` at Gate 3 (`gherkin-extractor.ts:189-193`).
...
Markdown prose containing the literal pattern `@architect-status:VALUE` is treated as a real
...
2. The regex `@architect-status:(\w+)` matches literal tag references in prose (lines 196, 301)
...
The bare tag name `@architect-status` (without `:value`) does not trigger the regex.
...
`@architect-target` format and were not affected.
...
constructing the full tag string `@architect-target` before searching the JSDoc comment text.
Description lines are never searched because they don't match the `@architect-` prefix pattern.
